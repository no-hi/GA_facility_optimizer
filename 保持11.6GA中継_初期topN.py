...
creator.create("Individual", list, fitness=creator.FitnessMin)#, inc_facility=None, trans_facility=None, unused_cities=None , id=None
...
def GA_count(N_INC, N_TRANS):
    ...
    with open(os.path.join(output_directory, f"output_{str(N_INC)+"&"+str(N_TRANS)}.txt"), 'a', encoding="utf-8") as f:  # 追記モードで開く
        ...
        def create_individual():
            ...
            print(individual.inc_facility)
            return individual



        # def individual_str(self):
        #     return f"ID: {self.id}, Inc Facility: {self.inc_facility}, Trans Facility: {self.trans_facility}, Unused Cities: {self.unused_cities}"

        # creator.Individual.__str__ = individual_str
