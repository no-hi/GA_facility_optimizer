...
filename = f"GA_graph_cost2D{goal.replace('[','').replace(']','').replace('inc','').replace('trans','')}_{current_time}.txt"
cost_component = costlist.cost_component
########################################################################

class FacilityListMerger:
    def __init__(self, cost_component, info_component, goal):   
    ...
    def merge_lists(self):
        ...
        for i, (info1, first_level_list1) in enumerate(zip(self.info_component, self.cost_component)):
            ...
            for j in range(len(self.info_component)):
                ...
                first_level_list2 = self.cost_component[j]
...
merger = FacilityListMerger(cost_component, info_component, goal)
