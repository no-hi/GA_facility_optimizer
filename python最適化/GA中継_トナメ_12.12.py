...
def GA_count(N_INC, N_TRANS):
    ...
    def create_individual():
        ...
        # 未使用の都市リストを計算
        individual.unused_cities = list(set(range(N_CITIES)) - set(individual.inc_facility) - set(individual.trans_facility))            
        return individual       
    ...
    def repair(individual):
        ...
        if duplicates:
            for facility in duplicates:
                ...
                for i in indices:
                    new_facility = random.choice(individual.unused_cities)
    ...
    def cxSet(ind1, ind2):
        ...
        ind1.unused_cities = list(set(range(N_CITIES)) - set(ind1))
        ind2.unused_cities = list(set(range(N_CITIES)) - set(ind2))            
    ...
    def mutSet(individual):
        ...
        for i in range(len(individual.inc_facility)):
            if random.random() < MUT_PROB:
                new_value = random.choice(individual.unused_cities)
                individual.unused_cities.append(individual.inc_facility[i])
        ...
        for i in range(len(individual.trans_facility)):
            if random.random() < MUT_PROB:
                new_value = random.choice(individual.unused_cities)
                individual.unused_cities.append(individual.trans_facility[i])
