...
def GA_count(N_INC, N_TRANS):
    ...
    with open(os.path.join(output_directory, f"output_{str(N_INC)+"&"+str(N_TRANS)}.txt"), 'a', encoding="utf-8") as f:  # 追記モードで開く
        ...
        def format_inc(facility_key, inc_size, yearly_trans_size, best_individual, cities_to_inc, trans_to_inc):
            ...
            if indirect_size > 0:
                trans_to_inc_names = ', '.join(f"{hokkaido[trans]}({yearly_trans_size[best_individual.trans_facility.index(trans)]})" for trans in trans_to_inc[facility_key])
                formatted_output.append(f"indirect {hokkaido[facility_key]}({indirect_size})/{inc_size} → receive from: {trans_to_inc_names}")
