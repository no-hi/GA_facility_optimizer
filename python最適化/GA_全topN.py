...
def get_top_cities(garbage_amounts, top_n=TOP_N_CITIES):
    # ごみ量上位N個
    return sorted(range(len(garbage_amounts)), key=lambda i: garbage_amounts[i], reverse=True)[:top_n]
...
def GA_count(N_FACILITIES):
    ...
    with open(os.path.join(output_directory, f"output_{N_FACILITIES}.txt"), 'a', encoding="utf-8") as f:  # 追記モードで開く
        ...
        # 個体、個体群
        top_cities = get_top_cities(garbage_amounts, top_n=TOP_N_CITIES)
        ...
        def cxSet(ind1, ind2):
            ...
            def repair(ind):
                ...
                # 修正点: missing が TOP_N_CITIES 以内に限定されるようにする
                missing = list(set(top_cities) - set(ind))
        ...
        def mutSet(individual):
            ...
            for i in range(len(temp)):
                if random.random() < MUT_PROB:
                    # 修正点: 未使用の市町村リストが TOP_N_CITIES 以内に限定されるようにする
                    unused_cities = set(top_cities) - set(temp)
        ...
        # コスト関数
        def total_cost(individual, garbage_amounts, distance):
            # !=はノットイコールと同意
            ...
            for city_i in range(N_CITIES):
                ...
                for faci_i in individual:
                    if distance[city_i][faci_i] < near_faci_distance:
                        ...
                        near_faci = faci_i
                # TC = garbage_amounts[city_i] * near_faci_distance * 300 / 10000
                # オーバーフロー要員（iburiの苫小牧一桁増やしたらオーバーフローしたので
                TC = float(garbage_amounts[city_i]) * float(near_faci_distance) * 300 / 10000  # 明示的に float に変換
                # print(f'garbage_amounts[{city_i}] = {garbage_amounts[city_i]}, near_faci_distance = {near_faci_distance}, TC = {TC}, dtype of TC = {type(TC)}')
                TC_values[near_faci] += TC
                total_TC += TC
                
                facility_size[individual.index(near_faci)] += garbage_amounts[city_i]
        ...
        def evaluate(individual):
            if len(set(individual)) < N_FACILITIES:
                ...
                total, _, _, _, _, _ = total_cost(individual, garbage_amounts, distance)
                return total,
        ...
        total, TC_values, IC_values, OC_values, facility_size, cities_to_facility = total_cost(best_individual, garbage_amounts, distance)

        output_content.append("\nBest Solution:")

        # ごみ量ソート
        top_cities_indices_and_amounts = [(i, garbage_amounts[i]) for i in get_top_cities(garbage_amounts, top_n=TOP_N_CITIES)]
