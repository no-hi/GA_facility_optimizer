...
def GA_count(N_INC, N_TRANS):
    ...
    with open(os.path.join(output_directory, "output_{0}.txt".format(str(N_INC)+"&"+str(N_TRANS))), 'a', encoding="utf-8") as f:  # 追記モードで開く        sys.stdout = f
        ...
        def create_individual():
            ...
            individual.inc_facility = random.sample(top_cities, N_INC)
            # 中継施設の遺伝子：すべての都市からランダムに選ばれる
            if N_TRANS > 0:  # 中継施設が0の場合は空のリストを割り当てる
                individual.trans_facility = random.sample(range(N_CITIES), N_TRANS)
            else:
        ...
        def total_cost(individual):
            ...
            return (total_TC + total_IC_inc + total_OC_inc + total_IC_trans + total_OC_trans, TC_direct_values, IC_inc_values, OC_inc_values, TC_indirect_values , IC_trans_values ,OC_trans_values , yearly_inc_size, yearly_trans_size , cities_to_inc , cities_to_trans , trans_to_inc)
        ...
        def evaluate(individual):
            if (len(set(individual.inc_facility)) + len(set(individual.trans_facility))) < N_INC + N_TRANS:
                ...
                total, _, _, _, _, _, _, _, _, _, _, _ = total_cost(individual)
                return total,
        ...
        output_file_path = "output_{0}.txt".format(str(len(best_individual.inc_facility))+"&"+str(len(best_individual.trans_facility)))
        ...
        total, TC_direct_values, IC_inc_values, OC_inc_values, TC_indirect_values , IC_trans_values ,OC_trans_values , yearly_inc_size, yearly_trans_size , cities_to_inc , cities_to_trans , trans_to_inc = total_cost(best_individual)
        output_content.append("\nBest Solution:")
        
        
        print("焼却施設:", best_individual.inc_facility)
        print("転送施設:", best_individual.trans_facility)
        print("年間焼却サイズ:", yearly_inc_size)
        
        
        # ごみ量ソート
        top_cities_i_and_amount = [(i, waste[i]) for i in get_top_cities()]
        top_cities_info = [f"{hokkaido[i]} ({amount})" for i, amount in top_cities_i_and_amount]
        ...
        for i, inc_size in sorted_inc_size:
            ...
            city_to_inc_names = ', '.join([hokkaido[city] for city in cities_to_inc[inc_key]])
            trans_to_inc_sizes_names = []
            for trans_key in trans_to_inc[inc_key]:
                trans_index = best_individual.trans_facility.index(trans_key)  # trans_key からインデックスを取得
                trans_size = yearly_trans_size[trans_index]
                trans_name = hokkaido[trans_key]
                trans_to_inc_sizes_names.append(f"{trans_name}({trans_size})")

            trans_to_inc_names = ', '.join(trans_to_inc_sizes_names)

            # 出力内容の追加
            output_content.append(f"direct {hokkaido[inc_key]}({inc_size - sum(yearly_trans_size[best_individual.trans_facility.index(trans)] for trans in trans_to_inc[inc_key])})/{inc_size} → receive from: {city_to_inc_names}")
        ...
        for i, trans_size in sorted_trans_size:
            trans_name = hokkaido[best_individual.trans_facility[i]]
            city_to_trans_names = ', '.join([hokkaido[city_to_trans] for city_to_trans in cities_to_trans[best_individual.trans_facility[i]]])
            output_content.append(f"{trans_name} ({trans_size}) → receive from: {city_to_trans_names}")

        # コスト
        sorted_TC_direct = {hokkaido[key]: TC_direct_values[key] for key in sorted_inc_i if key in TC_direct_values}
        sorted_IC_inc = {hokkaido[key]: IC_inc_values[key] for key in sorted_inc_i if key in IC_inc_values}
        sorted_OC_inc = {hokkaido[key]: OC_inc_values[key] for key in sorted_inc_i if key in OC_inc_values}
        output_content.append("\nTC_direct: " + str(sorted_TC_direct))
        output_content.append("IC_inc: " + str(sorted_IC_inc))
        output_content.append("OC_inc: " + str(sorted_OC_inc))
...
#     for count_trans in range(N_TRANS_MAX):
...
optimal_file_name = "output_{0}.txt".format(str(optimal_count_inc)+"&"+str(optimal_count_trans))
new_file_name = "output_{0}_best.txt".format(str(optimal_count_inc)+"&"+str(optimal_count_trans))
...
print("最適な焼却＆中継施設数: {0} での総コスト: {1}".format(str(optimal_count_inc)+"&"+str(optimal_count_trans), best_solutions[optimal_count_inc,optimal_count_trans]))
