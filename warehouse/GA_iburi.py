...
garbage_amounts = iburidata.kanen
...
def GA_count(N_FACILITIES):
    ...
    with open(os.path.join(output_directory, f"output_{N_FACILITIES}.txt"), 'a', encoding="utf-8") as f:  # 追記モードで開く
        ...
        # 個体、個体群
        toolbox.register("indices", random.sample, range(N_CITIES), N_FACILITIES)
        ...
        output_content.append("施設数: " + str(len(best_individual)))
        output_content.append("Selected facilities: " + ', '.join([hokkaido[i] for i in best_individual]))
        output_content.append("\nFacility size: " + str(facility_size))
        for facility, cities in cities_to_facility.items():
            city_names = ', '.join([hokkaido[city] for city in cities])
            output_content.append(f"Facility at {hokkaido[facility]} receives garbage from cities: {city_names}")
        output_content.append("\nTC: " + str(TC_values))
        output_content.append("\nIC: " + str(IC_values))
        output_content.append("\nOC: " + str(OC_values))
