...
def output_info(N_INC, N_TRANS, N_IND, get_top_cities, total_double_info, gen_info, sumgen, best_individual_after, start_time_count, current_time, output_directory, lock, double_2D, counter, localmark):    
    ...
    output_content += [header,
                    ...
                    f"個体数＝{str(N_IND)}",
                    f"合計世代数＝{str(sumgen)}",
                    "="*len(str("Total cost: ") + str(total_double_)),
                    "Total cost: " + str(total_double_),
                    "="*len(str("Total cost: ") + str(total_double_)),
                    ]
    ...
    output_content += ["------------------------  前提  ------------------------\n",
                    ...
                    f"輸送単価＝ {str(UNIT_energy_TRANS)} 円/t/km",
                    ]
    ...
    # GA_Graph用出力
    def extract_list(shared_list):  # 共有化されたdouble_2Dを通常リストに変換
    ...
    with lock: # 共有化されたcost2Dやparallel.counterをいじるときはlockをかける
        ...
        if counter[N_INC] == N_TRANS_MAX - N_TRANS_INITIAL + 1:
            ...
            if all_conditions_met:
                ...
                subprocess.run(["git", "commit", "-m", f"自動コミット（エネルギーevery_INC）:{UNIT_energy_TRANS}{waste_name}{N_INC_INITIAL}~{N_INC}&{N_TRANS_INITIAL}~{N_TRANS_MAX}"], check=False)
