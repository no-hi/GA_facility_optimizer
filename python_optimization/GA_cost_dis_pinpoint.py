...
# 並列実行########################################################################
def multi_task(task, current_time, output_directory, lock, cost_2D, counter, lock2):
    count_inc, count_trans = task
    best_individual = GA.GA_optimization(count_inc, count_trans, current_time, output_directory, lock, cost_2D, counter, lock2)
...
if __name__ == '__main__':
    try:
        ...
        lock = manager.Lock()
        lock2 = manager.Lock()
        cost_2D_origin = [[[] for _ in range(N_TRANS_INITIAL, N_TRANS_MAX + 1, 10)] for _ in range(N_INC_INITIAL, N_INC_MAX + 1, 10)]
        cost_2D = manager.list([manager.list([manager.list(item) for item in sublist]) for sublist in cost_2D_origin])    
        counter = manager.dict({i: 0 for i in range(N_INC_INITIAL, N_INC_MAX + 1, 10)})
        best_solutions = {(n_inc, n_trans): None for n_inc in range(N_INC_INITIAL, N_INC_MAX + 1, 10) for n_trans in range(N_TRANS_INITIAL, N_TRANS_MAX + 1, 10)}
        ...
        if restarting_output_directory == "":
            ...
            tasks = list(product(range(N_INC_INITIAL, N_INC_MAX + 1, 10), range(N_TRANS_INITIAL, N_TRANS_MAX + 1, 10)))
            ...
            for n_inc in range(N_INC_INITIAL, N_INC_MAX + 1, 10):
            ...
            tasks = [(count_inc, count_trans) for count_inc in range(N_INC_INITIAL, N_INC_MAX + 1, 10) for count_trans in range(N_TRANS_INITIAL, N_TRANS_MAX + 1, 10) if (count_inc, count_trans) not in completed_tasks]        
            # 未完了のタスクを計算負荷に基づいて分配
            num_processes = multiprocessing.cpu_count()
            distributed_tasks = distribute_tasks(tasks, num_processes)
        ...
        for i in range(0, len(cost_2D), group_size):
            ...
            for j in range(group_size):
                if i + j < len(cost_2D):
                    finish = cost_2D[i + j]
                    display_inc = float(10(i + j)) + N_INC_INITIAL
                    all_done = all(finish)  # すべてのトランザクションが完了しているかチェック
                    progress = [str(10*float(k) + N_TRANS_INITIAL) if finish[k] else "@" for k in range((N_TRANS_MAX - N_TRANS_INITIAL + 1)//10)]
