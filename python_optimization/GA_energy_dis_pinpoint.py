...
if __name__ == '__main__':
    try:
        ...
        if restarting_output_directory == "":
            ...
            subprocess.run(["git", "commit", "-m", f"自動コミット(エネルギースタート):{UNIT_TRANS}{waste_name}{N_INC_INITIAL}~{N_INC_MAX}&{N_TRANS_INITIAL}~{N_TRANS_MAX}"], check=False)
        ...
        if restarting_output_directory != "":
            def extract_list(shared_list):  # 共有化されたcost_2Dを通常リストに変換
                if isinstance(shared_list, multiprocessing.managers.ListProxy):
                    ...
                    return shared_list
            normal_cost_2D = extract_list(energy_2D)
