...
if horizon == "inc":
    ...
    for N_TRANS in range(len(transposed_cost)):
        ...
        title_suffix = f'N_TRANS = {N_TRANS*10}'
        xlabel = 'N_INC'
        filename = os.path.join(save_folder, f'TRANS{N_TRANS*10}.png')
...
if horizon == "trans":
    ...
    for N_INC in range(len(cost)):
        ...
        print(f"N_INC: {(N_INC+1)*10}, Selected data shape: {cost_per_inc.shape}")

        title_suffix = f'N_INC = {(N_INC+1)*10}'
        xlabel = 'N_TRANS'
        filename = os.path.join(save_folder, f'INC{(N_INC+1)*10}.png')
