...
import GA_function_double_pin.GA_output_display_double as output_display
import GA_function_double_pin.GA_local_double as local
...
UNIT_cost_TRANS2 = input.UNIT_cost_TRANS2
UNIT_energy_TRANS = input. UNIT_energy_TRANS
UNIT_energy_TRANS2 = input.UNIT_energy_TRANS2
...
def GA_optimization(N_INC, N_TRANS, current_time, output_directory, lock, double_2D, counter,energy_2D,cost_2D, lock2):
    ...
    def total_double(individual):
        def total_energy(individual):            
            def EL(individual):
                ...
                for city_i in range(N_CITIES):
                    ...
                    EL_direct = float(waste[city_i]/2) * (near_inc_distance*2) * 37.7/UNIT_energy_TRANS / 1000
                    ...
                    if N_TRANS > 0:
                        ...
                        EL_indirect = (float(waste[city_i]/2) * (near_trans_distance*2) /UNIT_energy_TRANS + float(waste[city_i]/10) * near_inc_from_trans_distance*2 /UNIT_energy_TRANS2) * 37.7/1000
        ...
        def total_cost(individual):            
            def TC(individual):
                ...
                for city_i in range(N_CITIES):
                    ...
                    near_inc_faci_i = min(range(len(inc_faci)), key=lambda x: distance[city_i][inc_faci[x]])
                    near_inc_distance = distance[city_i][inc_faci[near_inc_faci_i]]
                    TC_direct = float(waste[city_i]) * near_inc_distance * UNIT_cost_TRANS / 10000
                    ...
                    if N_TRANS > 0:
                        ...
                        near_inc_from_trans_faci_i = min(range(len(inc_faci)), key=lambda x: distance[trans_faci[near_trans_faci_i]][inc_faci[x]])
                        near_inc_from_trans_distance = distance[trans_faci[near_trans_faci_i]][inc_faci[near_inc_from_trans_faci_i]]
                        TC_indirect = (float(waste[city_i]) * near_trans_distance * UNIT_cost_TRANS + float(waste[city_i]) * near_inc_from_trans_distance * UNIT_cost_TRANS2) / 10000
        ...
        manyen_GJ = UNIT_double_TRANS /(3.6e6 * 1e-9) /10000 #ほくでん規制料金(2023)？＝43.68円/kWh , kWh/GJ = 3.6e6 * 1e-9
        total_double_ = total_cost_ + manyen_GJ*total_energy_
    
        return total_double_, yearly_inc_size, yearly_trans_size

    def total_double_info(best_individual):
    ...
    def evaluate(individual):
        if set(individual.inc_facility) & set(individual.trans_facility) or (len(set(individual.inc_facility)) + len(set(individual.trans_facility))) != N_INC + N_TRANS:
            ...
            total_double_, *_ = total_double(individual)          
            return total_double_,
    ...
    best_individual_after = local.local_optimization(best_individual, total_double, cities_zero)
