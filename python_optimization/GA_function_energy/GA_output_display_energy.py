...
#情報表示###############################################################################################################
def output_info(N_INC, N_TRANS, N_IND, get_top_cities, total_energy_info, gen_info, sumgen, best_individual_after, start_time_count, current_time, output_directory, lock, energy_2D, counter, localmark, lock2):    
    ...
    total_energy_, total_EL_direct, total_EL_indirect, total_ED_inc, total_ED_trans, EL_direct_values, ED_inc_values, EL_indirect_values , ED_trans_values , yearly_inc_size, yearly_trans_size , cities_to_inc , cities_to_trans , trans_to_inc = total_energy_info(best_individual_after)
    ...
    with lock: # 共有化されたcost2Dやparallel.counterをいじるときはlockをかける
        ...
        if counter[N_INC] == N_TRANS_MAX - N_TRANS_INITIAL + 1:
            ...
            with open(os.path.join(output_directory, f"GA_Graph({UNIT_TRANS}{waste_name}){current_time}.txt"), 'w', encoding="utf-8") as file:
                sequence = [i for i in range(N_INC_INITIAL, N_INC_MAX + 1) if all(counter[j] == N_TRANS_MAX - N_TRANS_INITIAL + 1 for j in range(N_INC_INITIAL, i + 1))]
                if sequence:
