    ...
    for n_trans in range(N_TRANS_INITIAL, N_TRANS_MAX + 1):
        ...
        if os.path.exists(filepath):
            ...
            if totalcost is not None:
                    total_2D[n_inc-N_INC_INITIAL][n_trans-N_TRANS_INITIAL] = totalcost
...
with open(os.path.join(output_directory, f"_totalcost({UNIT_TRANS}{waste_name}){current_time}.txt"), 'w', encoding="utf-8") as file:
    ...
    file.write(f"totalcost = {str(total_2D)}\n")