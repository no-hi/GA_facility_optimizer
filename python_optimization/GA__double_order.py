...
if __name__ == '__main__':
    try:
        ...
        if restarting_output_directory == "":
            ...
            subprocess.run(["git", "commit", "-m", f"自動コミット(コストスタート):{UNIT_double_TRANS}{waste_name}{N_INC_INITIAL}~{N_INC_MAX}&{N_TRANS_INITIAL}~{N_TRANS_MAX}"], check=False)
        ...
        subprocess.run(["git", "commit", "-m", f"自動コミット（コスト終了）:{UNIT_double_TRANS}{waste_name}{N_INC_INITIAL}~{N_INC_MAX}&{N_TRANS_INITIAL}~{N_TRANS_MAX}"], check=True)
